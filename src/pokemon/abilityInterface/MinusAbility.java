...
public class MinusAbility extends AbilityEffect {
	public MinusAbility(Pokemon owner) {
		super(owner);
	}

	@Override
	public void onSwitchIn(BattleContext battleCtx, Pokemon defender) {
		// Check if player has another Pokemon with "Plus" ability => informative
		// message
		if (owner.getOwner().getPokemon().stream().anyMatch(pk -> pk.hasPlusAbility()))
